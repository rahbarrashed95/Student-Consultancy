...
<main>
    <div class="container-fluid px-4">
        <h3 class="mt-4"> {{ __('report.Order Delivery Check') }} </h3>
        <ol class="breadcrumb mb-4">
            <li class="breadcrumb-item"><a href="{{ route('admin.dashboard')}}">{{ __('report.Dashboard') }}</a></li>
            <li class="breadcrumb-item active"> {{ __('report.Order Delivery Check Report') }} </li>
        </ol>
...
<script type="text/javascript">
  $(document).ready(function () {
    ...
    function getData(page=null){
        ...
        $('#order_data').html('');
        $.ajax({
            url: '{{ route("admin.reports.orderDeliveryCheck")}}?page='+page,
            type: 'GET',
            data:{q,start,end,status},
            dataType: 'json',
